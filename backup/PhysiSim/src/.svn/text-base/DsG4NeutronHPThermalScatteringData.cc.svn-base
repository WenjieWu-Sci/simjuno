...
DsG4NeutronHPThermalScatteringData::DsG4NeutronHPThermalScatteringData()
{
// Upper limit of neutron energy 
   ...
   names = new DsG4NeutronHPThermalScatteringNames();
...
void DsG4NeutronHPThermalScatteringData::clearCurrentXSData()
{
...
G4bool DsG4NeutronHPThermalScatteringData::IsApplicable( const G4DynamicParticle* aP , const G4Element* anEle )
{
   G4bool result = false;
...
void DsG4NeutronHPThermalScatteringData::BuildPhysicsTable(const G4ParticleDefinition& aP)
{
...
std::map< G4double , G4NeutronHPVector* >* DsG4NeutronHPThermalScatteringData::readData ( G4String name ) 
{
   ...
   //G4cout << "DsG4NeutronHPThermalScatteringData " << name << G4endl;
...
void DsG4NeutronHPThermalScatteringData::DumpPhysicsTable( const G4ParticleDefinition& aP )
{
   if( &aP != G4Neutron::Neutron() ) 
     throw G4HadronicException(__FILE__, __LINE__, "Attempt to use NeutronHP data for particles other than neutrons!!!");  
//  G4cout << "DsG4NeutronHPThermalScatteringData::DumpPhysicsTable still to be implemented"<<G4endl;
}
...
G4double DsG4NeutronHPThermalScatteringData::GetCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   G4double result = 0;
   ...
   //G4cout << "DsG4NeutronHPThermalScatteringData::GetCrossSection  Tot= " << result/barn << " Coherent= " << Xcoh/barn << " Incoherent= " << Xincoh/barn << " Inelastic= " << Xinela/barn << G4endl;
...
G4double DsG4NeutronHPThermalScatteringData::GetInelasticCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   G4double result = 0;
   G4int iele = anE->GetIndex();
...
G4double DsG4NeutronHPThermalScatteringData::GetCoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   G4double result = 0;
   G4int iele = anE->GetIndex();
...
G4double DsG4NeutronHPThermalScatteringData::GetIncoherentCrossSection( const G4DynamicParticle* aP , const G4Element*anE , G4double aT )
{
   G4double result = 0;
   G4int iele = anE->GetIndex();
...
G4double DsG4NeutronHPThermalScatteringData::GetX ( const G4DynamicParticle* aP, G4double aT , std::map < G4double , G4NeutronHPVector* >* amapTemp_EnergyCross )
{
   G4double result = 0;
   ...
   //G4cout << "DsG4NeutronHPThermalScatteringData::GetX TH " << TH << " E " << eKinetic <<  " XH " << XH << G4endl;

   it--;
   G4double TL = it->first;
   G4double XL = it->second->GetXsec ( eKinetic ); 

   //G4cout << "DsG4NeutronHPThermalScatteringData::GetX TL " << TL << " E " << eKinetic <<  " XL " << XL << G4endl;
